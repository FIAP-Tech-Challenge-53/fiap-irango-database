...
resource "aws_db_instance" "payment" {
  ...
  identifier = "${data.terraform_remote_state.infra.outputs.resource_prefix}-payment-rds"
  ...
  tags = {
    Name = "${data.terraform_remote_state.infra.outputs.resource_prefix}-payment-rds"
  }
}
